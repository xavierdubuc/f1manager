...
class LapRecordManager(AbstractManager):
    ...
    def create(cls, packet: PacketSessionHistoryData) -> LapRecord:
        ...
        if 1 <= packet.best_sector1_lap_num <= len(packet.best_sector1_lap_num):
        ...
        if 1 <= packet.best_sector2_lap_num <= len(packet.best_sector2_lap_num):
        ...
        if 1 <= packet.best_sector3_lap_num <= len(packet.best_sector3_lap_num):
    ...
    def update(cls, lap_record, packet: PacketSessionHistoryData) -> Dict[str, Change]:
        ...
        if 'best_sector3_lap_num' in changes:
            best_sector3_data = packet.lap_history_data[packet.best_sector3_lap_num-1]
            changes['best_sector3_time'] = Change(actual=best_sector3_data.sector3_time_in_ms, old=lap_record.best_sector3_time)
            lap_record.best_sector3_time = best_sector3_data.sector3_time_in_ms
        return changes