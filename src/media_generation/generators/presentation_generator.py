...
class PresentationGenerator(AbstractRaceGenerator):
    ...
    def _render_circuit_details(self, base_img: PngImageFile):
        ...
        total_length_content = total_length_config.get(
            'content', '{total_length}').format(total_length=self.race.get_total_length())
        total_length_txt = self.text(total_length_config.get(
            'text'), total_length_content, default_font=FontFactory.regular)
    ...
    def _render_last_winner(self, base_img: PngImageFile):
        ...
        pilot_name_txt = text_hi_res(pilot_name, pilot_name_color, pilot_name_font,
                                     pilot_name_w, pilot_name_h, use_background=bgcolor)
        ...
        season_content = season_config.get('content', '{season}').format(
            season=self.config.race.circuit_last_winner_season)
    ...
    def _render_fastest_lap(self, base_img: PngImageFile):
        ...
        if season := self.race.circuit_fastest_lap_season:
            ...
            season_txt = text_hi_res(season_content, season_color, season_font,
                                     season_w, season_h, use_background=bgcolor)
